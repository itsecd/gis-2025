...
# ===================== ПУТИ (исправлено под твою структуру) =====================
BASE_DIR      = os.path.dirname(os.path.abspath(__file__))  # C:/.../Lab_3
GEOJSON_IN    = os.path.join(BASE_DIR, 'map.geojson')       # C:/.../Lab_3/map.geojson
GEOJSON_OUT   = os.path.join(BASE_DIR, 'overture.geojson')  # C:/.../Lab_3/overture.geojson
CLIENT_PUBLIC = os.path.join(BASE_DIR, '..', 'Lab_2', 'client', 'public', 'overture.geojson')
# =================================================================================

RELEASE = '2026-04-15.0'
S3_BASE = f's3://overturemaps-us-west-2/release/{RELEASE}/theme=buildings/type=building'

# bbox по данным из map.geojson (Еланцы, Иркутская область)
MINX, MINY, MAXX, MAXY = 106.37, 52.78, 106.42, 52.82
...
def export_geojson(con: duckdb.DuckDBPyConnection) -> None:
    ...
    # Сохраняем в Lab_3/overture.geojson
    with open(GEOJSON_OUT, 'w', encoding='utf-8') as fh:
        json.dump(fc, fh, ensure_ascii=False, indent=2)
    
    # Сохраняем в Lab_2/client/public/overture.geojson
    try:
        os.makedirs(os.path.dirname(CLIENT_PUBLIC), exist_ok=True)
        with open(CLIENT_PUBLIC, 'w', encoding='utf-8') as fh:
            json.dump(fc, fh, ensure_ascii=False, indent=2)
        print(f'[3.4] Copied to {CLIENT_PUBLIC}')
    except Exception as e:
        print(f'[WARN] Could not copy to client: {e}')
...
def main() -> None:
    ...
    print(f'[INFO] Loading GeoJSON from: {GEOJSON_IN}')
    print(f'[INFO] Output will be saved to: {GEOJSON_OUT}')
